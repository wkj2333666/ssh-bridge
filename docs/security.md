...
No Codex credential, bridge package, plugin, or MCP server is placed on a server. On a supported Linux host the bridge uploads a local, static helper into a private mode-0700 per-session directory, verifies its helper handshake, and removes that directory at session exit; unsupported hosts use the shell dispatcher. The uploaded bytes are length-delimited stdin data and are never interpolated into shell source. All SSH authentication occurs in the local OpenSSH client.
